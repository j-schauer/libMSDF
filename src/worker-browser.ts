/**
 * Worker thread for parallel glyph generation (Browser Web Worker)
 */
...
async function initWorker(init: WorkerInit) {
    // Dynamically import the Emscripten module factory
    const { default: createModule } = await import(/* @vite-ignore */ init.wasmJsUrl);

    // Fetch WASM binary
    const response = await fetch(init.wasmBinaryUrl);
